...
UK_TRACEPOINT(trace_plat_vmware_unhandled_irq, "Unhandled irq=%lu\n",
	      unsigned long);
...
static struct irq_handler irq_handlers[__MAX_IRQ]
				[CONFIG_VMWARE_MAX_IRQ_HANDLER_ENTRIES];

static inline struct irq_handler *allocate_handler(unsigned long irq)
{
	UK_ASSERT(irq < __MAX_IRQ);
	for (int i = 0; i < CONFIG_VMWARE_MAX_IRQ_HANDLER_ENTRIES; i++)
...
void _ukplat_irq_handle(struct __regs *regs, unsigned long irq)
{
	...
	for (i = 0; i < CONFIG_VMWARE_MAX_IRQ_HANDLER_ENTRIES; i++) {
		if (irq_handlers[irq][i].func == NULL)
			break;
	 ...
	 */
	trace_plat_vmware_unhandled_irq(irq);
