		...
		struct virtio_vsock_packet *pkt)
{
	...
	if (unlikely(rc < 0)) {
		uk_pr_err("Failed to append the header buffer to the sglist.\n");
		return -ENOSPC;
	}
	...
	if (unlikely(rc < 0)) {
		uk_pr_err("Failed to append the data buffer to the sglist.\n");
		return -ENOSPC;
	}
				   ...
				   int notify)
{
	...
	for (i = 0; i < nb_desc; i++) {
		...
		if (unlikely(!pkt)) {
			uk_pr_err("Couldn't alloc rx packet\n");
			break;
		}
		...
		if (unlikely(!pkt->data)) {
			uk_pr_err("Couldn't alloc rx data\n");
			break;
		}
...
static int virtio_vsockdev_feature_negotiate(struct virtio_vsock_device *vskdev)
{
	...
	if (bytes_to_read != sizeof(guest_cid))  {
		uk_pr_err("bytes_to_read: %d, guest_cid: %d", bytes_to_read, sizeof(guest_cid));
	...
	drv_features = host_features;

	vskdev->vdev->features = drv_features;
	virtio_feature_set(vskdev->vdev, vskdev->vdev->features);

	virtio_dev_status_update(vskdev->vdev, (VIRTIO_CONFIG_STATUS_ACK | VIRTIO_CONFIG_STATUS_DRIVER | VIRTIO_CONFIG_STATUS_FEATURES_OK));
