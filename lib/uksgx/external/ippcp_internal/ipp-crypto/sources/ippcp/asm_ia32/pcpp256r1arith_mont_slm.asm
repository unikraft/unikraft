...
%assign LEN256  (256/32) ; dword's length of operands
...
;;
align IPP_ALIGN_FACTOR
IPPASM shl_256,PRIVATE
      ; r = a<<1
      movdqu   xmm0, oword [esi]
      movdqu   xmm1, oword [esi+sizeof(oword)]
      mov      eax, dword [esi+(LEN256-1)*sizeof(dword)]
...
; stack layout:
;
%assign _buf_  0                             ; buffer[LEN256]
%assign _sp_  _buf_+(LEN256)*sizeof(dword)  ; esp[1]
%assign _frame_  _sp_ +sizeof(dword)           ; +16 bytes for alignment
      ...
      mov      esi, pA                 ; pA
      mov      ebx, pB                 ; pB
      CALL_IPPASM  add_256                 ; R = A+B
      mov      edx, eax

      lea      edi, [esp+_buf_]        ; T
      mov      esi, pR                 ; R
      LD_ADDR  ebx, p256r1_data        ; modulus
      lea      ebx, [ebx+(_prime256r1-p256r1_data)]
      CALL_IPPASM  sub_256                 ; T = R-modulus
...
%assign _frame_  _sp_ +sizeof(dword)           ; +16 bytes for alignment
      ...
      LD_ADDR  ebx, p256r1_data        ; modulus
      lea      ebx, [ebx+(_prime256r1-p256r1_data)]
      CALL_IPPASM  add_256                 ; T = R+modulus
...
%assign _frame_  _sp_ +sizeof(dword)           ; +16 bytes for alignment
      ...
      LD_ADDR  ebx, p256r1_data        ; modulus
      lea      ebx, [ebx+(_prime256r1-p256r1_data)]
      CALL_IPPASM  sub_256                 ; R = T-modulus
...
; stack layout:
;
%assign _bufT_  0                             ; T buffer[LEN256]
%assign _bufU_  _bufT_+(LEN256)*sizeof(dword) ; U buffer[LEN256]
%assign _mod_  _bufU_+(LEN256)*sizeof(dword) ; modulus address [1]
...
%assign _frame_  _sp_ +sizeof(dword)           ; +16 bytes for alignment
      ...
      lea      eax, [eax+(_prime256r1-p256r1_data)]
      ...
      CALL_IPPASM  sub_256                 ; U = T-modulus
...
%assign _frame_  _sp_ +sizeof(dword)           ; +16 bytes for alignment
      ...
      LD_ADDR  ebx, p256r1_data        ; modulus
      lea      ebx, [ebx+(_prime256r1-p256r1_data)]
      CALL_IPPASM  add_256                 ; R = A+modulus
      mov      edx, 0
...
;;
align IPP_ALIGN_FACTOR
IPPASM p256r1_mul_mont_slm,PUBLIC
...
;
%assign _buf_  0
%assign _rp_  _buf_+(LEN256+1)*sizeof(dword) ; pR
...
;;; and reduction ;;;
      ...
      mov      dword [esp+_buf_+sizeof(dword)*1], ebx    ; copy
...
; final reduction
      ...
      lea      ebx, [ebx+(_prime256r1-p256r1_data)]
      CALL_IPPASM  sub_256
      mov      edx, dword [esp+_buf_+LEN256*sizeof(dword)]
      sub      edx, eax
...
;;
align IPP_ALIGN_FACTOR
IPPASM p256r1_sqr_mont_slm,PUBLIC
  USES_GPR esi,edi
...
%xdefine pA [esp + ARG_1 + 1*sizeof(dword)] ; source A address
      ...
      push  esi
      push  edi
      CALL_IPPASM  p256r1_mul_mont_slm,PUBLIC
      add   esp, sizeof(dword)*3
   REST_GPR
   ret
ENDFUNC p256r1_sqr_mont_slm
