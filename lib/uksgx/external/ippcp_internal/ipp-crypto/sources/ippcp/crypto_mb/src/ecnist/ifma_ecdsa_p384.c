...
*/
static __mb_mask nistp384_ecdsa_verify_mb8(U64 sign_r[],
                                           U64 sign_s[],
                                           U64 msg[],
                                   P384_POINT* W)
{
    /* convert public key coords to Montgomery */
    MB_FUNC_NAME(ifma_tomont52_p384_)(W->X, W->X);
    MB_FUNC_NAME(ifma_tomont52_p384_)(W->Y, W->Y);
    MB_FUNC_NAME(ifma_tomont52_p384_)(W->Z, W->Z);

    __ALIGN64 U64 h1[P384_LEN52];
    __ALIGN64 U64 h2[P384_LEN52];

    /* h = (sign_s)^(-1) */
    MB_FUNC_NAME(ifma_tomont52_n384_)(sign_s, sign_s);
    MB_FUNC_NAME(ifma_aminv52_n384_)(sign_s, sign_s);
    /* h1 = msg * h */
    MB_FUNC_NAME(ifma_tomont52_n384_)(h1, msg);
    MB_FUNC_NAME(ifma_amm52_n384_)(h1, h1, sign_s);
    MB_FUNC_NAME(ifma_frommont52_n384_)(h1, h1);
    /* h2 = sign_r * h */
    MB_FUNC_NAME(ifma_tomont52_n384_)(h2, sign_r);
    MB_FUNC_NAME(ifma_amm52_n384_)(h2, h2, sign_s);
    MB_FUNC_NAME(ifma_frommont52_n384_)(h2, h2);

    int64u tmp[8][P384_LEN64];
    int64u* pa_tmp[8] = { tmp[0], tmp[1], tmp[2], tmp[3],
                         tmp[4], tmp[5], tmp[6], tmp[7] };

    ifma_mb8_to_BNU(pa_tmp, (const int64u(*)[8])h1, P384_BITSIZE);
    ifma_BNU_transpose_copy((int64u(*)[8])h1, (const int64u(**))pa_tmp, P384_BITSIZE);

    ifma_mb8_to_BNU(pa_tmp, (const int64u(*)[8])h2, P384_BITSIZE);
    ifma_BNU_transpose_copy((int64u(*)[8])h2, (const int64u(**))pa_tmp, P384_BITSIZE);

    h1[P384_LEN64] = get_zero64();
    h2[P384_LEN64] = get_zero64();

    P384_POINT P;

    // P = h1*G + h2*W
    MB_FUNC_NAME(ifma_ec_nistp384_mul_point_)(W, W, h2);
    MB_FUNC_NAME(ifma_ec_nistp384_mul_pointbase_)(&P, h1);
    MB_FUNC_NAME(ifma_ec_nistp384_add_point_)(&P, &P, W);

    // P != 0
    __mb_mask signature_err_mask = MB_FUNC_NAME(is_zero_point_cordinate_)(P.Z);

    /* sign_r_restored = P.X mod n */
    __ALIGN64 U64 sign_r_restored[P384_LEN52];
    MB_FUNC_NAME(get_nistp384_ec_affine_coords_)(sign_r_restored, NULL, &P);
    MB_FUNC_NAME(ifma_frommont52_p384_)(sign_r_restored, sign_r_restored);
                                              ...
                                              int8u* pBuffer)
{
   ...
   U64 T[P384_LEN52];
   ifma_BNU_to_mb8((int64u (*)[8])T, pa_eph_skey, P384_BITSIZE);

   status |= MBX_SET_STS_BY_MASK(status, MB_FUNC_NAME(is_zero_FE384_)(T), MBX_STATUS_MISMATCH_PARAM_ERR);
   ...
   ifma_BNU_transpose_copy((int64u (*)[8])scalarz, pa_eph_skey, P384_BITSIZE);
                                                ...
                                                int8u* pBuffer)
{
   ...
   status |= MBX_SET_STS_BY_MASK(status, MB_FUNC_NAME(is_zero_FE384_)(inv_eph), MBX_STATUS_MISMATCH_PARAM_ERR);
   status |= MBX_SET_STS_BY_MASK(status, MB_FUNC_NAME(is_zero_FE384_)(reg_skey), MBX_STATUS_MISMATCH_PARAM_ERR);
   status |= MBX_SET_STS_BY_MASK(status, MB_FUNC_NAME(is_zero_FE384_)(sign_r), MBX_STATUS_MISMATCH_PARAM_ERR);
   status |= MBX_SET_STS_BY_MASK(status, MB_FUNC_NAME(ifma_check_range_n384_)(msg), MBX_STATUS_MISMATCH_PARAM_ERR);
   ...
   nistp384_ecdsa_sign_s_mb8(sign_s, msg, sign_r, inv_eph, reg_skey, pBuffer);
                                       ...
                                       int8u* pBuffer)
{
   ...
   status |= MBX_SET_STS_BY_MASK(status, MB_FUNC_NAME(is_zero_FE384_)(inv_eph_key), MBX_STATUS_MISMATCH_PARAM_ERR);
   status |= MBX_SET_STS_BY_MASK(status, MB_FUNC_NAME(is_zero_FE384_)(reg_key), MBX_STATUS_MISMATCH_PARAM_ERR);
   status |= MBX_SET_STS_BY_MASK(status, MB_FUNC_NAME(ifma_check_range_n384_)(msg), MBX_STATUS_MISMATCH_PARAM_ERR);
   ...
   /* compute s-component */
   nistp384_ecdsa_sign_s_mb8(sign_s, msg, sign_r, inv_eph_key, reg_key, pBuffer);
...
*/
DLL_PUBLIC
mbx_status mbx_nistp384_ecdsa_verify_mb8(const int8u* const pa_sign_r[8],
                                               ...
                                               int8u* pBuffer)
{
   ...
   /* test input pointers */
   if (NULL == pa_sign_r || NULL == pa_sign_s || NULL == pa_msg || NULL == pa_pubx || NULL == pa_puby) {
      status = MBX_SET_STS_ALL(MBX_STATUS_NULL_PARAM_ERR);
      return status;
   }
   ...
   /* check data pointers */
   for (buf_no = 0; buf_no < 8; buf_no++) {
      const int8u* psign_r = pa_sign_r[buf_no];
      const int8u* psign_s = pa_sign_s[buf_no];
      const int8u* pmsg = pa_msg[buf_no];
      ...
      /* if any of pointer NULL set error status */
      if (NULL == psign_r || NULL == psign_s || NULL == pmsg || NULL == pubx || NULL == puby || (use_jproj_coords && NULL == pubz)) {
         status = MBX_SET_STS(status, buf_no, MBX_STATUS_NULL_PARAM_ERR);
      }
   }
   ...
   ifma_HexStr8_to_mb8((int64u(*)[8])msg, pa_msg, P384_BITSIZE);
   ifma_HexStr8_to_mb8((int64u(*)[8])sign_r, pa_sign_r, P384_BITSIZE);
   ifma_HexStr8_to_mb8((int64u(*)[8])sign_s, pa_sign_s, P384_BITSIZE);
   ...
   */
   status |= MBX_SET_STS_BY_MASK(status, MB_FUNC_NAME(ifma_check_range_n384_)(msg), MBX_STATUS_MISMATCH_PARAM_ERR);
   status |= MBX_SET_STS_BY_MASK(status, MB_FUNC_NAME(ifma_check_range_p384_)(sign_r), MBX_STATUS_MISMATCH_PARAM_ERR);
   status |= MBX_SET_STS_BY_MASK(status, MB_FUNC_NAME(ifma_check_range_n384_)(sign_s), MBX_STATUS_MISMATCH_PARAM_ERR);
   ...
   ifma_BNU_to_mb8((int64u(*)[8])W.X, (const int64u* (*))pa_pubx, P384_BITSIZE);
   ifma_BNU_to_mb8((int64u(*)[8])W.Y, (const int64u* (*))pa_puby, P384_BITSIZE);
   if (use_jproj_coords)
      ifma_BNU_to_mb8((int64u(*)[8])W.Z, (const int64u* (*))pa_pubz, P384_BITSIZE);
   else
      MB_FUNC_NAME(mov_FE384_)(W.Z, (U64*)ones);

   status |= MBX_SET_STS_BY_MASK(status, MB_FUNC_NAME(ifma_check_range_p384_)(W.X), MBX_STATUS_MISMATCH_PARAM_ERR);
   status |= MBX_SET_STS_BY_MASK(status, MB_FUNC_NAME(ifma_check_range_p384_)(W.Y), MBX_STATUS_MISMATCH_PARAM_ERR);
   status |= MBX_SET_STS_BY_MASK(status, MB_FUNC_NAME(ifma_check_range_p384_)(W.Z), MBX_STATUS_MISMATCH_PARAM_ERR);

   if (!MBX_IS_ANY_OK_STS(status))
   return status;

   __mb_mask signature_err_mask = nistp384_ecdsa_verify_mb8(sign_r, sign_s, msg, &W);
                                                  ...
                                                  int8u* pBuffer)
{
   ...
   /* convert keys into FE and compute inversion */
   U64 T[P384_LEN52];
   ifma_BN_to_mb8((int64u (*)[8])T, pa_eph_skey, P384_BITSIZE);
   ...
   /* convert keys into scalars */
   U64 scalarz[P384_LEN64+1];
   ifma_BN_transpose_copy((int64u (*)[8])scalarz, pa_eph_skey, P384_BITSIZE);
   scalarz[P384_LEN64] = get_zero64();
   /* compute r-component of the DSA signature */
   int8u stt_mask = nistp384_ecdsa_sign_r_mb8(T, scalarz, pBuffer);
                                                    ...
                                                    int8u* pBuffer)
{
   ...
   /* convert inv_eph, reg_skey, sign_r and message to mb format */
   ifma_BN_to_mb8((int64u (*)[8])inv_eph, pa_inv_eph_skey, P384_BITSIZE);
   ifma_BN_to_mb8((int64u (*)[8])reg_skey, pa_reg_skey, P384_BITSIZE);
                                           ...
                                           int8u* pBuffer)
{
   ...
   scalar[P384_LEN64] = get_zero64();
   /* convert reg_skey*/
   ifma_BN_to_mb8((int64u (*)[8])reg_key, pa_reg_skey, P384_BITSIZE);
   /* convert message */
   ifma_HexStr8_to_mb8((int64u (*)[8])msg, pa_msg, P384_BITSIZE);
                                                       ...
                                                       int8u* pBuffer)
{
   ...
   /* test input pointers */
   if (NULL == pa_msg || NULL == pa_sig || NULL == pa_pubx || NULL == pa_puby) {
      status = MBX_SET_STS_ALL(MBX_STATUS_NULL_PARAM_ERR);
      return status;
   }
   ...
   for (buf_no = 0; buf_no < 8; buf_no++) {
      ...
      /* if any of pointer NULL set error status */
      if (NULL == sig || NULL == pmsg || NULL == pubx || NULL == puby || (use_jproj_coords && NULL == pubz)) {
         status = MBX_SET_STS(status, buf_no, MBX_STATUS_NULL_PARAM_ERR);
      }
   }
   ...
   ifma_BN_to_mb8((int64u(*)[8])W.X, pa_pubx, P384_BITSIZE);
   ifma_BN_to_mb8((int64u(*)[8])W.Y, pa_puby, P384_BITSIZE);
   if (use_jproj_coords)
      ifma_BN_to_mb8((int64u(*)[8])W.Z, pa_pubz, P384_BITSIZE);
   else
      MB_FUNC_NAME(mov_FE384_)(W.Z, (U64*)ones);
