...
%macro SHA1_UPDATE_RND_F1_BSWAP 7.nolist
  ...
  %xdefine %%Wchunk %7
   ...
   pshufb   %%Wchunk, XMM_SHUFB_BSWAP
   movdqa   W, %%Wchunk
   paddd    %%Wchunk, oword [K_XMM]
   movdqa   oword [rsp + (%%nr & 15)*4], %%Wchunk
   mov      T1,%%C   ; F1
   mov      T2,%%A
   xor      T1,%%D   ; F1
   and      T1,%%B   ; F1
  ...
  %xdefine K_XMM  K_BASE+48
   ...
   %if (%%i < 32)
      ...
      %elif ((%%i & 3) == 3)
         ...
         paddd    W_TMP, oword [K_XMM]
         movdqa   oword [rsp + ((%%i & (~3)) & 15)*4],W_TMP
   ...
   %elif (%%i < 83)
      %if ((%%i & 3) == 0)  ;; scheduling to interleave with ALUs
         movdqa   W_TMP, W_minus_04
         pxor     W, W_minus_28      ;; W == W_minus_32
