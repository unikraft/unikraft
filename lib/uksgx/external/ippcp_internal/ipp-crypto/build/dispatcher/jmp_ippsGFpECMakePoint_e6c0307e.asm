%if ((__NASM_MAJOR__ > 2) || ((__NASM_MAJOR__ == 2) && (__NASM_MINOR__ > 14)))
  %xdefine elf_symbol_type :function
%else
  %xdefine elf_symbol_type
%endif
extern m7_ippsGFpECMakePoint%+elf_symbol_type
extern n8_ippsGFpECMakePoint%+elf_symbol_type
extern y8_ippsGFpECMakePoint%+elf_symbol_type
extern e9_ippsGFpECMakePoint%+elf_symbol_type
extern l9_ippsGFpECMakePoint%+elf_symbol_type
extern n0_ippsGFpECMakePoint%+elf_symbol_type
extern k0_ippsGFpECMakePoint%+elf_symbol_type
extern k1_ippsGFpECMakePoint%+elf_symbol_type
extern ippcpJumpIndexForMergedLibs
extern ippcpSafeInit%+elf_symbol_type


segment .data
align 8
dq  .Lin_ippsGFpECMakePoint
.Larraddr_ippsGFpECMakePoint:
    dq m7_ippsGFpECMakePoint
    dq n8_ippsGFpECMakePoint
    dq y8_ippsGFpECMakePoint
    dq e9_ippsGFpECMakePoint
    dq l9_ippsGFpECMakePoint
    dq n0_ippsGFpECMakePoint
    dq k0_ippsGFpECMakePoint
    dq k1_ippsGFpECMakePoint

segment .text
global ippsGFpECMakePoint:function (ippsGFpECMakePoint.LEndippsGFpECMakePoint - ippsGFpECMakePoint)
.Lin_ippsGFpECMakePoint:
    db 0xf3, 0x0f, 0x1e, 0xfa
    call ippcpSafeInit wrt ..plt
    align 16

ippsGFpECMakePoint:
    db 0xf3, 0x0f, 0x1e, 0xfa
    mov     rax, qword [rel ippcpJumpIndexForMergedLibs wrt ..gotpc]
    movsxd  rax, dword [rax]
    lea     r11, [rel .Larraddr_ippsGFpECMakePoint]
    mov     r11, qword [r11+rax*8]
    jmp     r11
.LEndippsGFpECMakePoint:
