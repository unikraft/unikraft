...
#if CONFIG_LIBUKTTY_COM_EARLY_CONSOLE
...
#endif /* CONFIG_LIBUKLIBPARAM */
#endif /* CONFIG_LIBUKTTY_COM_EARLY_CONSOLE */
...
int com_early_init(struct ukplat_bootinfo *bi __unused)
{
	...
	early_dev.dev = UK_CONSOLE(COM_EARLY_NAME, &com_ops,
			 ...
			 __u8 bauddiv_lo, __u8 bauddiv_hi)
{
	...
	com_dev->port = port;
	com_dev->dev = UK_CONSOLE(name, &com_ops, 0);
...
#if CONFIG_LIBUKTTY_COM1 && !CONFIG_LIBUKTTY_COM1_EARLY
	rc = com_init_port(a, "COM1", COM1_PORT,
			   COM1_BAUDDIV_LO, COM_BAUDDIV_HI);
	if (unlikely(rc < 0))
		return rc;
#endif /* CONFIG_LIBUKTTY_COM1 && !CONFIG_LIBUKTTY_COM1_EARLY */
#if CONFIG_LIBUKTTY_COM2 && !CONFIG_LIBUKTTY_COM2_EARLY
	rc = com_init_port(a, "COM2", COM2_PORT,
			   COM2_BAUDDIV_LO, COM_BAUDDIV_HI);
	if (unlikely(rc < 0))
		return rc;
#endif /* CONFIG_LIBUKTTY_COM2 && !CONFIG_LIBUKTTY_COM2_EARLY */
#if CONFIG_LIBUKTTY_COM3 && !CONFIG_LIBUKTTY_COM3_EARLY
	rc = com_init_port(a, "COM3", COM3_PORT,
			   COM3_BAUDDIV_LO, COM_BAUDDIV_HI);
	if (unlikely(rc < 0))
		return rc;
#endif /* CONFIG_LIBUKTTY_COM3 && !CONFIG_LIBUKTTY_COM3_EARLY */
#if CONFIG_LIBUKTTY_COM4 && !CONFIG_LIBUKTTY_COM4_EARLY
	rc = com_init_port(a, "COM4", COM4_PORT,
			   COM4_BAUDDIV_LO, COM_BAUDDIV_HI);
	if (unlikely(rc < 0))
		return rc;
#endif /* CONFIG_LIBUKTTY_COM4 && !CONFIG_LIBUKTTY_COM4_EARLY */
