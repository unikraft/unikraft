...
static int early_init(struct ukplat_bootinfo *bi)
{
	...
	earlycon.dev = UK_CONSOLE("PL011", &pl011_ops,
			  ...
			  int *offset)
{
	...
	*offset = fdt_node_offset_by_compatible_list(dtb, *offset,
	...
	dev->dev = UK_CONSOLE("PL011", &pl011_ops, 0);
	dev->base = reg_base;
	dev->size = reg_size;
...
static int init(struct uk_init_ctx *ictx __unused)
{
	...
	while (1) {
		pl011_dev = uk_malloc(a, sizeof(*pl011_dev));
		if (unlikely(!pl011_dev)) {
			uk_pr_err("Could not allocate pl011 device\n");
			return -ENOMEM;
		}

		rc = fdt_get_device(pl011_dev, dtb, &offset);
...
#if CONFIG_PAGING
		/* Map device region */
		base = uk_bus_pf_devmap(pl011_dev->base, pl011_dev->size);
		 ...
		 */
		if (pl011_dev->base == earlycon.base) {
		...
		uk_console_register(&pl011_dev->dev);
		uk_pr_info("tty: pl011 (%p)\n", &pl011_dev->base);
	}
